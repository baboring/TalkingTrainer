...
namespace TTrainer
{
	...
	public static class Logger
	{
		...
		public static void Init(string _path, string _FileName,string _header = ">")
		{
			if (!isInitialized) {
				isInitialized = true;
		...
		static bool IsEnableFileWrite = true;
		// 로그파일 생성
		static bool OpenLogFile(string _path, string _fileName)
		{
			// 폴더 생성 처리
			if (!string.IsNullOrEmpty(_path) && !System.IO.Directory.Exists(_path)) {
		...
		public static void Info(ColorType color, string logString)
		{
			WriteFileLog(logString, LogLevel.INFO);
